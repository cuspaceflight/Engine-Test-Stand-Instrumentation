...
--- v3.1 ADDITIONS (safety) -------------------------------------------------
Three additive commands and one message support a firmware-side fail-safe.
They are backward compatible at the wire level (new verbs only), BUT the
v3.1 firmware REQUIRES the v3.1 GUI: the firmware now fails safe if it does
not hear from the GUI. Deploy firmware and GUI together — see CODE_OVERVIEW.

    GUI → ESP32   "PING\\n"     liveness keepalive (GUI sends ~2 Hz). Silent
                               on the firmware side — produces no reply, so
                               it never disturbs the OK/ERROR ack stream.
    GUI → ESP32   "ABORT\\n"    atomic safe-state: firmware drives ALL servos
                               to their configured safe position and LATCHES
                               a fail-safe state (servo moves rejected) until
                               re-armed. Replies "OK".
    GUI → ESP32   "RESET\\n"    re-arm after an ABORT / comms-loss latch.
                               Replies "OK" and "STATE:ARMED".
    ESP32 → GUI   "STATE:ARMED\\n"            firmware is armed (servos live)
    ESP32 → GUI   "STATE:FAILSAFE:<reason>\\n" firmware is latched safe
                               (reason e.g. "boot", "abort", "linkloss").
                               Sent on every transition and repeated at 1 Hz
                               so a GUI that connects mid-stream learns state.
-----------------------------------------------------------------------------
...
ESP32 → GUI (Messages):
    ...
    STATE:ARMED\\n / STATE:FAILSAFE:<reason>\\n   arm/fail-safe state (v3.1)
    STATUS:SRV1:<µs>:...:CAN:<OK|FAULT>:STATE:<ARMED|FAILSAFE>\\n
"""
...
# ── v3.1 safety commands ─────────────────────────────────────────────────────

@dataclass
class PingCommand(Command):
    """Liveness keepalive. The GUI sends this on a timer (~2 Hz) so the
    firmware's operator-link watchdog can tell the GUI is still alive even
    when the operator is not clicking anything. The firmware does not reply,
    so PING never appears in the OK/ERROR ack stream."""
...
class AbortCommand(Command):
    """Atomic abort. One frame on the wire; the firmware drives all servos
    to their configured safe state and latches a fail-safe state. Preferred
    over sending four individual SRV:500 commands, which can partially fail."""
...
class ResetCommand(Command):
    """Re-arm after an abort or a comms-loss latch. The firmware will only
    accept this if the operator link is currently alive."""
...
class StateMessage(Message):
    """Arm / fail-safe state of the firmware (v3.1).

    state:  "ARMED" or "FAILSAFE"
    reason: free-text reason for a FAILSAFE state ("boot", "abort",
            "linkloss", ...); empty for ARMED.
    """
    state: str
    reason: str = ""
...
def parse_response(line: str) -> Message:
    ...
    # ── STATE:ARMED  /  STATE:FAILSAFE:<reason> ──  (v3.1)
    if line.startswith("STATE:"):
        parts = line.split(":", 2)   # at most 3 fields: STATE, state, reason
        state = parts[1] if len(parts) > 1 else ""
        reason = parts[2] if len(parts) > 2 else ""
        return StateMessage(state=state, reason=reason)
